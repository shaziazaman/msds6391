...
function loadWeatherData(cityName, unitSelected) {
	...
	var svg = d3.select("body").select("div#dashboard").append("svg").attr('width',490).attr('height',320);
...
function generateAndLoadLineChart(data, dashboard_svg){
	...
	// define dimensions of graph
	  ...
	  // plot with data json array passed, we'll plot with a line (this array represents only the Y values, X will just be the index location)
	    ...
	    render(data2,x_range,y_range,m,w,h,x_dim_accessor,y_dim_accessor, dashboard_svg);

}

function render(data,x_range,y_range,m,w,h,x_dim_accessor,y_dim_accessor, dashboard_svg){
